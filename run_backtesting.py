...
from src.analysis.risk_metrics import calculate_portfolio_risk_report, calculate_var_cvar, calculate_max_drawdown, calculate_downside_deviation
...
def generate_regime_wise_risk_report(df: pd.DataFrame, n_components: int) -> pd.DataFrame:
    ...
    for s in range(n_components):
        label = df[df['regime_state'] == s]['regime_label'].iloc[0] if s in df['regime_state'].values else ""
    ...
    for s in range(n_components):
        ...
        downside_vol = float(calculate_downside_deviation(ret))
        
        # Historical VaR/CVaR (95%)
        var_95, cvar_95 = calculate_var_cvar(ret, 0.95)
        # Max drawdown
        max_dd = float(calculate_max_drawdown(ret))
        ...
        trans_risk = float(np.sum(trans_matrix[s, bearish_states])) if bearish_states else 0.0
...
def main():
    ...
    if not os.path.exists(labeled_parquet_path):
        logger.error(f"Regime-labeled data Parquet not found at: {labeled_parquet_path}. Please run run_modeling.py first.")
        return
    ...
    n_components = int(labeled_df['regime_state'].nunique())
    
    # Ensure ret_simple is in labeled_df
    ...
    logger.info(f"NIFTY 50 baseline risk report saved to: {os.path.abspath(risk_report_path)}")
    ...
    logger.info(f"Regime-wise risk intelligence breakdown saved to: {os.path.abspath(regime_risk_path)}")
    ...
    equity_curves_df, backtest_summary_df = backtester.backtest_strategies(labeled_df, args.config)
    ...
    logger.info(f"Portfolio strategies backtesting summary saved to: {os.path.abspath(summary_path)}")
    ...
    logger.info(f"Daily portfolio equity curves saved to: {os.path.abspath(curves_parquet_path)}")
    ...
    logger.info(f"Historical stress test comparison saved to: {os.path.abspath(stress_report_path)}")
    ...
    for label, steps in horizons.items():
        logger.info(f"Simulating {args.simulations} paths over {label} horizon ({steps} days)...")
    ...
    mc_cols = ["horizon_label", "horizon_days", "paths_simulated", "expected_return", 
               "median_return", "worst_5pct_return", "best_5pct_return", 
               "probability_of_loss", "probability_of_drawdown_10pct", 
               "probability_of_drawdown_20pct", "average_simulated_max_drawdown"]
    ...
    logger.info(f"Markov-chain Monte Carlo path projections saved to: {os.path.abspath(mc_summary_path)}")
