...
def main():
    ...
    if not os.path.exists(features_parquet):
        logger.error(f"Features Parquet not found at: {features_parquet}. Please run the feature pipeline first.")
        return
    ...
    best_model_type, best_k, best_metrics = run_model_selection_sweep(features_df, args.config)
    ...
    logger.info(f"Training final {best_model_type} model on all data with {best_k} components...")
    
    final_model = None
    states = None
    
    if best_model_type == "HMM":
        final_model = GaussianHMM(n_components=best_k, covariance_type="diag", random_state=42, n_iter=200)
        final_model.fit(X)
        states = final_model.predict(X)
    elif best_model_type == "GMM":
        final_model = GMMRegimeModel(n_components=best_k, covariance_type="diag", random_state=42)
        final_model.fit(X)
        states = final_model.predict(X)
    else:
        # Fallback to Markov Switching
        log_ret = np.log(features_df['raw_close'] / features_df['raw_close'].shift(1)).fillna(0.0).values
        final_model = MarkovSwitchingModel(n_components=best_k)
        final_model.fit(log_ret)
        states = final_model.predict(log_ret)
    ...
    logger.info(f"Trained model binary serialized to: {os.path.abspath(model_save_path)}")
    ...
    logger.info(f"Regime-labeled dataset saved to: {os.path.abspath(labeled_parquet_path)}")
    
    # Save summaries
    summary_csv_path = os.path.join(reports_dir, "regime_summary.csv")
    summary_df.to_csv(summary_csv_path, index=False)
    logger.info(f"Regime statistical summary saved to: {os.path.abspath(summary_csv_path)}")
    
    # Save transition probabilities
    trans_csv_path = os.path.join(reports_dir, "transition_matrix.csv")
    trans_df.to_csv(trans_csv_path)
    logger.info(f"Transition probability matrix saved to: {os.path.abspath(trans_csv_path)}")
    ...
    logger.info(f"Updated selected model report with out-of-sample stability metrics: {os.path.abspath(report_json_path)}")
