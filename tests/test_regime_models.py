...
from src.models.hmm_model import CustomGaussianHMM, GaussianHMM
...
def test_regime_labeling_logic():
    ...
    assert labels[2] == "Bullish Low Volatility"
...
def test_regime_analysis_module(dummy_features):
    ...
    labeled_df, summary_df, trans_df = analyze_regimes(
        dummy_features, states, n_components=3
    )
...
def test_kmeans_baseline(dummy_features):
    """Tests that KMeans baseline fits, predicts, and generates pseudo-probabilities."""
    from src.models.kmeans_model import KMeansRegimeModel

    X = dummy_features[["feat_0", "feat_1", "feat_2"]].values

    model = KMeansRegimeModel(n_components=3, random_state=42)
    model.fit(X)
...
def test_reproducible_model_seed(dummy_features):
    """Verifies that setting a random seed results in deterministic predictions."""
    X = dummy_features[["feat_0", "feat_1", "feat_2"]].values

    model1 = GaussianHMM(n_components=3, random_state=42)
    model1.fit(X)
    states1 = model1.predict(X)

    model2 = GaussianHMM(n_components=3, random_state=42)
    model2.fit(X)
...
def test_model_small_dataset_graceful():
    """Verifies that models handle very small datasets gracefully without throwing exceptions."""
    from src.models.kmeans_model import KMeansRegimeModel

    X_small = np.random.normal(0, 1, (5, 3))

    # GMM
    model_gmm = GMMRegimeModel(n_components=2, random_state=42)
    model_gmm.fit(X_small)
    assert len(model_gmm.predict(X_small)) == 5

    # HMM
    model_hmm = GaussianHMM(n_components=2, random_state=42)
    model_hmm.fit(X_small)
    assert len(model_hmm.predict(X_small)) == 5

    # KMeans
    model_km = KMeansRegimeModel(n_components=2, random_state=42)
    model_km.fit(X_small)
    assert len(model_km.predict(X_small)) == 5
