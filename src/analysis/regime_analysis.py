...
def assign_regime_labels(regime_stats: List[Dict[str, Any]]) -> List[str]:
    ...
    sorted_by_ret = sorted(
        range(n_components), key=lambda i: regime_stats[i]["annualized_return"]
    )

    # Sort indices by volatility
    sorted_by_vol = sorted(
        range(n_components), key=lambda i: regime_stats[i]["annualized_volatility"]
    )
    ...
    if n_components == 2:
        ...
        labels[idx_bear] = (
            "Bearish High Volatility"
            if regime_stats[idx_bear]["annualized_return"] < 0
            else "Sideways Low Volatility"
        )
        ...
        sharpe_ranks = sorted(
            range(n_components),
            key=lambda i: regime_stats[i]["sharpe_ratio"],
            reverse=True,
        )
        ...
        highest_ret_remain = next(
            idx for idx in reversed(sorted_by_ret) if idx not in assigned
        )
...
def analyze_regimes(
    df: pd.DataFrame, states: np.ndarray, n_components: int
    ...
    # Run consecutive runs to compute average state durations (ignoring warmup states)
    consecutive_runs = {i: [] for i in range(n_components)}
    valid_states = [s for s in states if s >= 0]
    if valid_states:
        current_state = valid_states[0]
        current_run = 1
        for t in range(1, len(valid_states)):
            if valid_states[t] == current_state:
                current_run += 1
            else:
                consecutive_runs[current_state].append(current_run)
                current_state = valid_states[t]
                current_run = 1
        consecutive_runs[current_state].append(current_run)
    ...
    label_map = {
        item["regime_state"]: item["regime_label"] for item in regime_summaries
    }
    label_map[-1] = "Warmup / Undefined"
...
def run_walk_forward_validation(
    ...
    # Clean NaNs to prevent fitting failures
    clean_df = features_df.dropna(subset=feature_cols).copy()
    if clean_df.empty:
        logger.warning(
            "No valid clean rows for walk-forward validation. Returning empty report."
        )
        return {
            "regime_stability": [],
            "average_return_delta": 0.0,
            "average_volatility_delta": 0.0,
            "is_stable_oos": False,
        }
    ...
    if "raw_close" in clean_df.columns:
        ret_series = clean_df["raw_close"].pct_change(fill_method=None).fillna(0.0)
    else:
        ret_series = clean_df["close"].pct_change(fill_method=None).fillna(0.0)
    ...
    for i in range(n_components):
        ...
        train_vol = (
            ret_train[train_mask].std() * np.sqrt(252.0) if train_count > 0 else 0.0
        )
    ...
    avg_ret_delta = np.mean(
        [item["return_stability_delta"] for item in stability_report]
    )
