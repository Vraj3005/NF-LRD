...
def compute_rolling_slope(series: pd.Series, window: int) -> pd.Series:
    ...
    series.rolling(window=window).sum()
...
def compute_trends(
    df: pd.DataFrame, sma_windows: List[int], ema_windows: List[int]
) -> pd.DataFrame:
    """
    ...
    if 50 in sma_windows and 200 in sma_windows:
        feats["cross_sma_50_200"] = np.where(
            feats["sma_50"] > feats["sma_200"], 1.0, -1.0
        )
    else:
    ...
    cols_to_keep = [
        c for c in feats.columns if "dist" in c or "slope" in c or "cross" in c
    ]
