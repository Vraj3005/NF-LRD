...
def normalize_columns(df: pd.DataFrame, config_path: str = "config/settings.yaml") -> pd.DataFrame:
    """
    ...
    if 'date' not in df_clean.columns:
        ...
        if isinstance(df_clean.index, pd.DatetimeIndex):
            df_clean = df_clean.reset_index().rename(columns={"index": "date", "Date": "date"})
            ...
            logger.warning("Could not find mapped date column, trying to guess by column type/name.")
            for col in df_clean.columns:
                if "date" in col.lower() or "time" in col.lower() or "record" in col.lower():
                    df_clean = df_clean.rename(columns={col: "date"})
                    break

    # If date is still not found, raise a ValueError
    if 'date' not in df_clean.columns:
        raise ValueError(f"Could not locate a valid 'date' column in DataFrame. Available columns: {list(df_clean.columns)}")
    ...
    numeric_cols = [col for col in ['open', 'high', 'low', 'close', 'volume', 'turnover'] if col in df_clean.columns]
...
def merge_and_align_vix(
    ...
    if vix_df is None or vix_df.empty:
        logger.warning("No VIX data provided for alignment. Proceeding with NIFTY data only.")
        return nifty_df
    ...
    if missing_vix_before > 0:
        ...
        logger.info(f"Forward-filled VIX: {missing_vix_before - missing_vix_after} rows filled.")
...
def process_and_save_data(
    ...
    if not is_valid:
        logger.warning(
            f"Validation issues discovered. Details: {report.details}. "
            "Saving data anyway, but with warnings."
        )
    else:
