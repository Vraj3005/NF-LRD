...
def plot_regime_distributions(labeled_df: pd.DataFrame) -> go.Figure:
    ...
    if "ret_simple" not in labeled_df.columns:
        if "raw_close" in labeled_df.columns:
            labeled_df["ret_simple"] = labeled_df["raw_close"].pct_change(
                fill_method=None
            )
        else:
    ...
    fig.update_layout(
        showlegend=False, xaxis={"categoryorder": "total median ascending"}
    )
    return apply_chart_theme(fig)


def plot_monte_carlo_paths(
    paths: np.ndarray, dates_horizon: Optional[List[Any]] = None
) -> go.Figure:
    """
    ...
    fig.add_trace(
        go.Scatter(x=x_steps, y=p95, mode="lines", line=dict(width=0), showlegend=False)
    )
    fig.add_trace(
        go.Scatter(
            x=x_steps,
    ...
    fig.add_trace(
        go.Scatter(x=x_steps, y=p75, mode="lines", line=dict(width=0), showlegend=False)
    )
    fig.add_trace(
        go.Scatter(
            x=x_steps,
    ...
    for i in range(min(10, n_sims)):
        fig.add_trace(
            go.Scatter(
                ...
                y=paths[:, i],
                mode="lines",
                line=dict(width=0.8, color=path_color),
                name=f"Sim Path {i+1}",
                showlegend=False,
            )
        )
...
def plot_monte_carlo_distributions(paths: np.ndarray) -> go.Figure:
    ...
    fig.add_vline(
        ...
        line_color="red",
        line_width=2,
        annotation_text=f"Worst 5% tail: {tail_5pct:.1f}%",
...
def plot_rolling_risk_metrics(
    returns: pd.Series, benchmark_returns: Optional[pd.Series] = None, window: int = 63
) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="Annualized Volatility (%)", overlaying="y", side="right"),
        hovermode="x unified",
        template="plotly_dark",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
...
def plot_regime_overlaid_price(labeled_df: pd.DataFrame) -> go.Figure:
    ...
    for start_idx, end_idx in zip(block_starts, block_ends):
        ...
        label = (
            sub_df["regime_label"].iloc[0]
            if "regime_label" in sub_df.columns
            else f"State {state}"
        )
...
def plot_pca_2d(labeled_df: pd.DataFrame) -> go.Figure:
    ...
    feature_cols = [
        ...
        if c not in ["date", "regime_state", "regime_label", "ret_simple"]
    ...
    if len(df_clean) < 10:
        ...
        fig = go.Figure()
        fig.update_layout(
            title="PCA 2D Projection (Insufficient Data)", template="plotly_dark"
        )
        return fig
...
def plot_correlation_heatmap(labeled_df: pd.DataFrame) -> go.Figure:
    ...
    if not cols:
        cols = [
            c
            for c in labeled_df.columns
            if c not in ["date", "regime_state", "regime_label"]
            and not c.startswith("raw_")
        ][:10]
...
def plot_regime_durations(labeled_df: pd.DataFrame) -> go.Figure:
    ...
    durations = (
        df.groupby(["block_id", "regime_label"]).size().reset_index(name="duration")
    )
...
def plot_regime_probabilities_chart(
    dates: pd.Series, probs: np.ndarray, state_labels: List[str]
) -> go.Figure:
    """
...
def plot_pca_3d(labeled_df: pd.DataFrame) -> go.Figure:
    ...
    if len(df_clean) < 10:
        fig = go.Figure()
        fig.update_layout(
            title="PCA 3D Projection (Insufficient Data)", template="plotly_dark"
        )
        return fig
...
def plot_monthly_returns_heatmap(
    ...
    """
    Calculates monthly compounded returns and plots them as a heatmap.
    """
    df_m = pd.DataFrame({"ret": returns})
    df_m.index = pd.to_datetime(df_m.index)
    df_m["year"] = df_m.index.year
    df_m["month"] = df_m.index.month

    # Compound return per month
    monthly_grouped = df_m.groupby(["year", "month"])["ret"].apply(
        lambda x: np.prod(1.0 + x) - 1.0
    )
    monthly_table = monthly_grouped.unstack(level=1)
    ...
    monthly_table = monthly_table.rename(columns=month_names)

    # Fill missing month names
    for m_name in month_names.values():
        if m_name not in monthly_table.columns:
            monthly_table[m_name] = 0.0
    ...
    monthly_table = monthly_table.fillna(0.0)

    fig = px.imshow(
        monthly_table.values * 100.0,
...
def plot_rolling_sharpe(returns_dict: dict, window: int = 63) -> go.Figure:
    """
    Plots the rolling Sharpe ratio comparison for multiple strategies.
    """
    fig = go.Figure()
    for name, series in returns_dict.items():
        rolling_mean = series.rolling(window).mean() * 252
        rolling_std = series.rolling(window).std() * np.sqrt(252) + 1e-15
    ...
    fig.update_layout(
        title=f"Rolling {window}-Day Sharpe Ratio (Annualized)",
        xaxis_title="Date",
        yaxis_title="Sharpe Ratio",
        template="plotly_dark",
        height=350,
...
def plot_strategy_exposure(weights_dict: dict) -> go.Figure:
    """
    Plots portfolio allocation exposure over time.
    """
    fig = go.Figure()
    ...
    fig.update_layout(
        title="Tactical Asset Exposure Weights Over Time",
        xaxis_title="Date",
        yaxis_title="Portfolio Exposure (1.0 = 100%)",
        template="plotly_dark",
        height=350,
        margin=dict(l=20, r=20, t=40, b=20),
    )
